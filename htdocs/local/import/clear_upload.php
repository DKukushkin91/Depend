...
$rootDirPath = $_SERVER['DOCUMENT_ROOT'] . "/upload/main";
...
$arFilesCache = array();
$result = $DB->Query('SELECT FILE_NAME, SUBDIR FROM b_file WHERE MODULE_ID = "main"');
...
while (false !== ($subDirName = readdir($hRootDir))) {
    if ($subDirName == '.' || $subDirName == '..' || $subDirName == '.DS_Store') {
        continue;
    }
    ...
    while (false !== ($fileName = readdir($hSubDir))) {
        if ($fileName == '.' || $fileName == '..' || $subDirName == '.DS_Store') {
            continue;
        }
        $countFile++;
